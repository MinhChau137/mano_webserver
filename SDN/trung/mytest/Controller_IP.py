...
class ProjectController(app_manager.RyuApp):
    ...
    def get_link_cost(self, s1, s2): # chi phi giua 1 duong link
        '''
    ...
    def get_path_cost(self, path): # chi phi cua 1 tuyen duong
        '''
        Get the path cost
        '''
        cost = 0
    ...
    def sorted_path(self,paths,pw): # sap xep tu cost be nhat den lon nhat
    ...
    def get_optimal_paths(self, src, dst): # toi uu duong
        '''
    ...
    def add_ports_to_paths(self, paths, first_port, last_port): # port cua switch
        '''
        ...
        for node in paths[0]:
            ...
            for in_port in ports:
                ...
                # Ipv4
                    ...
                    # self.logger.info("tCP PORT: %s" % dst_port)
                    match_ip = ofp_parser.OFPMatch(
                        eth_type=0x0800,
                        ip_proto=ip_proto,
                        ipv4_src=ip_src, 
                    ...
                    # self.logger.info("tCP PORT: %s" % match_ip)
    ...
    def add_flow_notimeout(self, datapath, priority, match, actions, buffer_id=None):
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,
                                             actions)]
        if buffer_id:
            mod = parser.OFPFlowMod(datapath=datapath, buffer_id=buffer_id,idle_timeout=30, hard_timeout=0,
                                    priority=priority, match=match,
                                    instructions=inst)

        else:
            mod = parser.OFPFlowMod(datapath=datapath, priority=priority,idle_timeout=30, hard_timeout=0,
                                    match=match, instructions=inst)
    ...
    def _switch_features_handler(self, ev): # khi khoi tao Ryu
        ...
        self.add_flow_notimeout(datapath, 0, match, actions)

        list_ip = ["34.1.1.1","35.1.1.1"]
        mask = "255.0.0.0"
        for i in list_ip:
            match = parser.OFPMatch(
                        eth_type=0x0806,  
            ...
            actions = []
            self.add_flow_notimeout(datapath, 65535, match, actions)
        ...
        # x = requests.get('http://127.0.0.1:8000/refreshdata')
    ...
    #     except:
    #         self.logger.info("SQL config error!!!")
    #         pass
    ...
    def _packet_in_handler(self, ev):
        # self.logger.info("Co ban tin den")
        self.count += 1
        msg = ev.msg
        ...
        #     self.logger.info("Ban tin IPv4")

        # avoid broadcast from LLDP
        if eth.ethertype == ether_types.ETH_TYPE_LLDP:
            # self.logger.info("Drop")
            # self.logger.info("%s", eth)
            return
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            if ip_proto == 6:
                ...
                # self.logger.info("udp_pkt: %s" % udp_pkt)
        ...
        if isinstance(ip_pkt,ipv4.ipv4):
            ...
            if dst_ip in self.arp_table:
                ...
                # dict_value = [src_ip, dst_ip, src_port, dst_port]
                # mydict = {k:v for k,v in zip(dict_key,dict_value)}
                # self.logger.info("%s", self.kubeSystem)
                # json_merge = json.dumps(mydict)

                # post = requests.post(url = "http://127.0.0.1:8000/read", data = json_merge)

                # result_post = post.text
                # # self.logger.info("%s", result_post)
                # # self.logger.info("%s", type(result_post))
                # if (str(result_post) == "khongtontai"):
                ...
                # self.logger.info("In thanh cong")

                # x = requests.get('http://127.0.0.1:5000/test')
                # self.logger.info("%s", x.text)
                
                # if dst_ip in self.kubeSystem or src_ip in self.kubeSystem :
                #     out_port = self.install_paths(h1[0], h1[1], h2[0], h2[1], src_ip, dst_ip,ip_proto,ethvx_src,ethvx_dst,dst_port,src_ip, dst_ip, vni)
                #     self.install_paths(h2[0], h2[1], h1[0], h1[1], dst_ip, src_ip,ip_proto,ethvx_dst,ethvx_src,dst_port,src_ip, dst_ip, vni) # reverse
                # else:
                dict_key = ["ip_src", "ip_dst","port_src", "port_dst"]
                ...
                if (str(result_post) == "khongtontai"):
                    out_port = self.install_paths(h1[0], h1[1], h2[0], h2[1], src_ip, dst_ip,ip_proto,ethvx_src,ethvx_dst,dst_port,src_ip, dst_ip, vni)
                    self.install_paths(h2[0], h2[1], h1[0], h1[1], dst_ip, src_ip,ip_proto,ethvx_dst,ethvx_src,dst_port,src_ip, dst_ip, vni) # reverse
                    self.logger.info("Flow normal")
                else:
                    out_port = int(result_post)
                    self.logger.info("Da them trong Database.Route")
        ...
        # self.logger.info("KetThuc %s %s", in_port, out_port)
        if (out_port == ofproto.OFPP_FLOOD):
            self.logger.info("FLOOD: %s", pkt)
        
            
        

    @set_ev_cls(event.EventSwitchEnter)
    def switch_enter_handler(self, ev): # luu tru thong tin cua switch (Lay topo)
        switch = ev.switch.dp
    ...
    def switch_leave_handler(self, ev): # switch roi di
        print(ev)
    ...
    def link_add_handler(self, ev): # lien quan den Topo
        s1 = ev.link.src
        s2 = ev.link.dst
    ...
    def _port_stats_reply_handler(self, ev): # Stat cua port
    ...
    #     self.logger.info("path 1-3 %s\n\tcost:%s"%(p,pw))
        ...
        #     self.curr_paths.setdefault(i,{})
        #     if self.curr_paths[i]:
        #         self.logger.info('\t CURR_paths of switch %s is : %s\n \tcost: %s'%(i,self.curr_paths[i],self.curr_pw[i]))
...
# "\t\t_____________________________RESETPATH_______________________________\n\n")
    ...
    def get_ip_from_dpid(self,dpid):
        ...
        for host in hosts:
            ...
            # self.logger.info("HOST:%s"%a)

            # 1 host has only 1 IP
            # ip.append(a[0])

            # 1 host has multiple IP
            for i in a:
                ip.append(i)
        return ip
