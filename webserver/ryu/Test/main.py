...
class ExampleSwitch13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth_pkt.ethertype == ether_types.ETH_TYPE_IP:
            protocol = pkt.get_protocol(ipv4.ipv4).proto
            ip = pkt.get_protocol(ipv4.ipv4)
            srcip =ip.src
            dstip=ip.dst
            if protocol == in_proto.IPPROTO_TCP:
                t = pkt.get_protocol(tcp.tcp)
                port_nguon = t.src_port
                port_dich = t.dst_port

                self.logger.info("    IP packet in %s %s %s %s", srcip, dstip, port_nguon, port_dich)
                self.logger.info("                 ")
