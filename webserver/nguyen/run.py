...
def pod_deployment(data, test):
    ...
    for i in allService:
        ...
        selector_matchLabels_deploy = {
            "app": name + "-deployment", "ID": data['idt']}

        labels_metadata_template_deploy = {
            "app": name + "-deployment",  "ID": data['idt']}
        ...
        portContainerList = [client.V1ContainerPort(container_port=int(
            portContainerHead)), client.V1ContainerPort(container_port=int(9900))]
        ...
        deploy_spec = client.V1DeploymentSpec(
            ...
            selector=client.V1LabelSelector(
                match_labels=selector_matchLabels_deploy),  # ???? #sua lan 1
            template=deploy_template)

        deploy = client.V1Deployment(kind="Deployment",
                                     metadata=client.V1ObjectMeta(
                                         name=name_metadata_deploy),
                                     spec=deploy_spec)
        a += 1
        # deploy_result=appsv1.create_namespaced_deployment(namespace=data['idt'], body=deploy)
        # pprint(deploy_result)
        try:
            deploy_result = appsv1.create_namespaced_deployment(
                namespace=data['idt'], body=deploy)
            # a = deploy_result
...
def service_deployment(data, test):
    ...
    for i in allService:
        ...
        service_spec = client.V1ServiceSpec(type="ClusterIP",
                                            ports=[client.V1ServicePort(target_port=int(
                                                port_ports_spec), port=int(target_port_port))],
        ...
        try:
            service_result = corev1.create_namespaced_service(
                namespace=data['idt'], body=service)
            # pprint(service_result)
        except:
            return test
    test = True
...
def launch():
    ...
    # Tao File JSON:
        allService = data['service']
        find = 'SELECT ip_pod FROM pod.Pods where namespace = "%s" ' % (
            data['idt'])
        find_name = 'SELECT name_pod FROM pod.Pods where namespace = "%s" ' % (
            data['idt'])
        ...
        for i in list(route.keys()):
            ...
            if check == 2:
                ...
                value_count += 1
        print(route_dict)
        my_json = ({'idt': data["idt"], 'service': svc_list,
                   'ip_pod': ip_list, 'route': route_dict})
